...
*/
	...
	α OdbcDataSource::ExecuteProcNoLog( string sql, vec<object> v, SL sl )noexcept(false)->uint{ return ExecDirect( CS(),move(sql), nullptr, &v, sl, false ); }
	α OdbcDataSource::ExecuteProc( string sql, const vector<object>& parameters, SL sl )noexcept(false)->uint{ return Select( format("{{call {} }}", sql), nullptr, &parameters, sl); }
	...
	α OdbcDataSource::SelectCo( ISelect* pAwait, string sql_, vector<object>&& parms_, SL sl )noexcept->up<IAwaitable>
	{ 
		return mu<FunctionAwaitable>( [pAwait,sql=move(sql_),parms=move(parms_),this]( HCoroutine h )mutable->Task2
		{
			try
			{
				auto pBindings = parms.size() ? mu<vector<up<Binding>>>() : up<vector<up<Binding>>>{};
				if( pBindings )
				{
				...
				auto pStatement = ( co_await Execute(move(*pSession), string(sql), move(pBindings)) ).Get<HandleStatementAsync>();
				//if( f )
